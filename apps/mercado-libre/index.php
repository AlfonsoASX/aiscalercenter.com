...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    try {
        ...
        $providerConfig = is_array($config['providers']['mercado_libre'] ?? null) ? $config['providers']['mercado_libre'] : [];
        $provider = new MercadoLibreProvider($providerConfig, new HttpClient((int) ($config['http_timeout'] ?? 12)), new TextAnalyzer());
...
<div class="research-tool-page">
    <section class="research-tool-card">
        <div class="research-tool-card-head">
            ...
            <h2>Mercado Libre</h2>
            <p>Consulta senales de demanda y terminos relacionados desde Mercado Libre.</p>
        </div>
        ...
        <form method="post" action="<?= htmlspecialchars('tool.php?launch=' . rawurlencode((string) ($toolContext['launch_token'] ?? '')), ENT_QUOTES, 'UTF-8'); ?>" class="research-tool-form">
            ...
            <div class="research-tool-input-shell">
                ...
                <button type="submit" class="research-tool-submit">
                    ...
                    <span>Investigar en Mercado Libre</span>
                </button>
            </div>
        </form>
        ...
        <?php if (is_array($result)): ?>
            <?= renderResearchProviderResult('Mercado Libre', $idea, $result); ?>
        <?php else: ?>
            <div class="research-tool-empty">
                <span class="material-symbols-rounded">manage_search</span>
                <h3>Empieza con una idea</h3>
                <p>Escribe una idea y revisa terminos relacionados obtenidos desde Mercado Libre.</p>
            </div>
        <?php endif; ?>
    </section>
</div>
